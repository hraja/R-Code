...
setwd("/Users/hraja/Documents/DataProjects/RAG/Connor")

# reading tarocash SCV
ConnorSCV  = read.csv("PurchaseBehaviour-2.csv",header = TRUE,sep='|',stringsAsFactors = FALSE)

#reading SubscriberStats
subscriberStats = read.csv('SubStatsConnor.csv',header = TRUE,sep='|',stringsAsFactors = FALSE)
...
#K-means clustering with 6 clusters of sizes 82093, 178927, 337311, 49189, 7816, 71444

#Cluster means:
#         CLV   Frequency     Recency         AOS TotalClickRate TotalOpenRate
#1 -0.09837561 -0.02857141 -0.37470355 -0.11026083     0.47013477    2.26375564 (window shopper)
#2 -0.41382029 -0.37484357  1.36636242 -0.22174618    -0.21163537   -0.41010942 (lapse buyer)
#3 -0.19835948 -0.08320281 -0.50063099 -0.31259851    -0.15864349   -0.33865551 (rookie buyer)
#4  2.50078123  2.41992978 -0.81605301  0.11856451     0.03248015    0.07028912 (dream customer)
#5 -0.13136534 -0.13373989 -0.38934709 -0.03961875     7.79104755    2.07459146 (researcher)
#6  0.37853747 -0.28705341 -0.02332291  2.08062997    -0.13587974   -0.25053290 (occasional buyer)

# adding clusters to the cluster set

ClusterSet$cluster = seg.k6$cluster
write.csv(ClusterSet,"ConnorSegments.csv",row.names = FALSE)
